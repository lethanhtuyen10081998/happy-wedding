...
import { Button } from 'src/components/tiptap/components/ui/Button';
import { Icon } from 'src/components/tiptap/components/ui/Icon';
import { Loader } from 'src/components/tiptap/components/ui/Loader';
import { Panel, PanelHeadline } from 'src/components/tiptap/components/ui/Panel';
import { Textarea } from 'src/components/tiptap/components/ui/Textarea';
...
export const AiImageView = ({ editor, node, getPos, deleteNode }: NodeViewWrapperProps) => {
  ...
  const insert = useCallback(() => {
    ...
    editor.chain().insertContent(`<img src="${previewImage}" alt="" />`).deleteRange({ from: getPos(), to: getPos() }).focus().run();
      ...
      <Panel noShadow className='w-full'>
        <div className='flex flex-col p-1'>
          ...
          <div className='flex flex-row items-center justify-between gap-1'>
            ...
            <div className='flex flex-row items-center justify-between gap-1'>
              {previewImage && (
                <Button variant='ghost' className='text-red-500 hover:bg-red-500/10 hover:text-red-500' onClick={discard}>
                  <Icon name='Trash' />
                  Discard
                </Button>
              )}
